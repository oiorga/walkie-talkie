...
sealed class ModuleOp<TOpId, TChId, TMsg> {
    sealed class Get<TOpId, V>: ModuleOp<TOpId, Nothing, Nothing>() {
        data class Request<TOpId, V> (val opId: TOpId): Get<TOpId, V>()
    }

    sealed class Set<TOpId, V>: ModuleOp<TOpId, Nothing, Nothing>() {
        data class Value<TOpId, V> (val opId: TOpId, val value: V): Set<TOpId, V>()
    }

    sealed class SubscribeEvent<TOpId, TChId, TMsg>: ModuleOp<TOpId, TChId, TMsg>() {
    ...
    sealed class SendEvent<TOpId, TChId, TMsg>: ModuleOp<TOpId, TChId, TMsg>() {
    ...
    data class Input<I>(val value: I) : ModuleOp<Nothing, Nothing, Nothing>()
    sealed class Output<out O>: ModuleOp<Nothing, Nothing, Nothing>() {
